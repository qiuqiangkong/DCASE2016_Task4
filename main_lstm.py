...
SUMMARY:  Dcase 2016 Task 4. Audio Tagging
          ...
          test f_value: 77% (thres=0.5), test EER=80%  after 100 epoches
...
md.add( GRU( n_out=50, act='tanh' ) )      # Try SimpleRnn, LSTM, GRU instead
md.add( GlobalMeanTimePool( masking=None ) )
...
# fit model
md.fit( x=tr_X, y=tr_y, batch_size=500, n_epoch=100, loss_type='binary_crossentropy', optimizer=optimizer, callbacks=callbacks )
